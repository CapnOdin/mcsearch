...
from . import searchable, nbt_util
...
class Chunk(searchable.Searchable, anvil.Chunk):
	...
	def __check_tags(self, tags, keys, verbose = 0):
		for key in keys:
			data = self.__lookup_names_in_tag(key, self.data) if type(key) is list else self.data[key]
	...
	def __lookup_names_in_tag(self, names, tag):
		res_tag = tag
		for name in names:
